...
import io.github.hsm7.superheroes.statistics.domain.Score;
...
/**
 * Consumer of {@link AvroFight} events from Kafka. There are 2 consumers for performing different aggregations. Each consumer writes out to its own in-memory channel.
 */
@ApplicationScoped
public class SuperStats {
    ...
    /**
     * Processes a stream of {@link AvroFight}s. Computes {@link #computeTeamStats(io.quarkus.sample.superheroes.fight.schema.AvroFight) team stats}
    ...
    @Broadcast
    public Multi<Message<Iterable<Score>>> processFight(Multi<Message<AvroFight>> fights) {
    ...
    /**
     * Transforms the {@link AvroFight} stream into a stream of {@link TeamScore scores}.
     * Each score indicates the running percentage of battles won by heroes.
     *
     * @param fight The {@link AvroFight} to compute
    ...
    Uni<Void> computeTeamStats(@SpanAttribute("arg.fight") AvroFight fight) {
    ...
    /**
     * Transforms the {@link AvroFight} stream into a running stream of top winners.
     * <p>
     * The incoming stream is first grouped by {@link AvroFight#getWinnerName}. Then the number of wins for that winner is computed.
     * </p>
     *
     * @param fights The {@link AvroFight} continuous stream
     * @return A continuous stream of the top 10 winners and the number of wins for each winner
     */
    private Multi<Iterable<Score>> computeTopWinners(Multi<AvroFight> fights) {
        return fights
                .withContext((multi, ctx) -> {
                            ...
                            return multi
                                    ...
                                    .group().by(AvroFight::getWinnerName)
                                    .flatMap(group ->
    ...
    private Multi<AvroFight> createTracedMulti(Multi<Message<AvroFight>> fights, Context ctx) {
        return fights
                .map(m -> {
    ...
    private Score incrementScore(Score score, AvroFight fight) {
    ...
    private static Message<Iterable<Score>> scoresMessage(Iterable<Score> scores, Context ctx) {
        Message<AvroFight> message = ctx.get(MESSAGE_KEY);
    ...
    private Span createChildSpan(String spanName, AvroFight fight, Optional<Span> parentSpan) {
        return createSpan(spanName, fight, getOtelContext(parentSpan));
    }

    private Span createSpan(String spanName, AvroFight fight, Optional<io.opentelemetry.context.Context> parent) {
    ...
    private Optional<Span> handleTrace(Message<AvroFight> message, String spanName) {
